...
/// Poster 容器背景扩展（从 App/Core/Views/Poster/View+Background.swift 迁移，移除 MagicKit 依赖）。
public extension View {
    func inPosterContainer() -> some View {
        GeometryReader { geo in
            self
                ...
                .background {
                    ZStack {
                        Circle()
                            .fill(LinearGradient(colors: [.blue.opacity(0.1), .cyan.opacity(0.06)], startPoint: .topLeading, endPoint: .bottomTrailing))
                            .frame(width: geo.size.width * 0.35, height: geo.size.width * 0.35)
                            .position(x: geo.size.width * 0.15, y: geo.size.height * 0.2)

                        Circle()
                            .fill(LinearGradient(colors: [.green.opacity(0.1), .mint.opacity(0.06)], startPoint: .topLeading, endPoint: .bottomTrailing))
                            .frame(width: geo.size.width * 0.3, height: geo.size.width * 0.3)
                            .position(x: geo.size.width * 0.85, y: geo.size.height * 0.15)

                        Circle()
                            .fill(LinearGradient(colors: [.purple.opacity(0.1), .pink.opacity(0.06)], startPoint: .topLeading, endPoint: .bottomTrailing))
                            .frame(width: geo.size.width * 0.4, height: geo.size.width * 0.4)
                            .position(x: geo.size.width * 0.8, y: geo.size.height * 0.75)

                        Circle()
                            .fill(LinearGradient(colors: [.orange.opacity(0.1), .yellow.opacity(0.06)], startPoint: .topLeading, endPoint: .bottomTrailing))
        ...
        .background(LinearGradient(colors: [Color.blue.opacity(0.15), Color.green.opacity(0.12), Color.purple.opacity(0.15)], startPoint: .topLeading, endPoint: .bottomTrailing))
        .background(.black)
        .colorScheme(.dark)
    }
}
