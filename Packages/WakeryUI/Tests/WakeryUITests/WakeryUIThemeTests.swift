...
private struct TestTheme: WakeryUITheme {
    let id = "test-theme"
    let name = "Test Theme"
...
struct WakeryUIThemeTests {
    @Test
    @MainActor
    func defaultThemeIsAvailable() {
        WakeryUI.setTheme(WakeryDefaultTheme())

        #expect(WakeryUI.currentTheme.id == "wakery-default")
        #expect(WakeryUI.currentTheme.name == "Wakery Default")
    }

    @Test
    @MainActor
    func setThemeReplacesCurrentTheme() {
        WakeryUI.setTheme(TestTheme())
        defer { WakeryUI.setTheme(WakeryDefaultTheme()) }

        #expect(WakeryUI.currentTheme.id == "test-theme")
        #expect(WakeryUI.currentTheme.name == "Test Theme")
    }
    ...
    func protocolExtensionGradientDefaultsAreAccessibleWithoutCrash() {
        ...
        // the LinearGradient fall-through paths in WakeryUITheme.
    ...
    @Test
    @MainActor
    func wakeryThemePropertyWrapperReflectsCurrentStore() {
        WakeryUI.setTheme(TestTheme())
        defer { WakeryUI.setTheme(WakeryDefaultTheme()) }

        let wrapper = WakeryTheme()
        #expect(wrapper.wrappedValue.id == "test-theme")
    }

    @Test
    @MainActor
    func wakeryDefaultThemeExposesExpectedIdentifiers() {
        let theme = WakeryDefaultTheme()

        #expect(theme.id == "wakery-default")
        #expect(theme.name == "Wakery Default")
    }
}
