...
@MainActor
public final class WakeryUIThemeRegistry: ObservableObject {
    public static let shared = WakeryUIThemeRegistry()

    @Published public private(set) var catalog: ThemeCatalog?

    @Published public private(set) var chromeTheme: any WakeryAppChromeTheme = UnconfiguredChromeTheme()

    @Published public private(set) var uiTheme: any WakeryUITheme = WakeryDefaultTheme()

    public init() {}

    public var themes: [WakeryUIThemeContribution] {
        catalog?.themes ?? []
    }

    public var selectedContribution: WakeryUIThemeContribution? {
        catalog?.selected
    }
    ...
    public func replaceAll(_ contributions: [WakeryUIThemeContribution]) throws {
    ...
    private func sortedUnique(_ contributions: [WakeryUIThemeContribution]) throws -> [WakeryUIThemeContribution] {
    ...
    private func applySelection() throws {
        ...
        uiTheme = resolvedUI
        WakeryUIThemeStore.shared.setTheme(resolvedUI)
    }
}
