...
public struct AppActivityIconButton: View {
    ...
    @WakeryMotionPreferenceReader private var motionPreference
        ...
        Button(action: action) {
            ZStack(alignment: .leading) {
                ...
                Image(systemName: systemImage)
                    ...
                    .scaleEffect(isHovered && !isActive && motionPreference.allowsMotion ? WakeryMotion.hoverScale : 1.0)
            }
        }
        .buttonStyle(.plain)
        .help(label)
        .animation(WakeryMotion.enabled(WakeryMotion.selection, preference: motionPreference), value: isActive)
        .onHover { hovering in
            WakeryMotion.animate(WakeryMotion.enabled(WakeryMotion.hover, preference: motionPreference)) {
                isHovered = hovering
            }
        }
    }
